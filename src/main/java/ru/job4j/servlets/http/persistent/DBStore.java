...
public class DBStore implements Store {
    ...
    private DBStore() {
        ...
        config.init();
        SOURCE.setDriverClassName(this.config.get("jdbc.driver"));
        SOURCE.setUrl(this.config.get("jdbc.url"));
        SOURCE.setUsername(this.config.get("jdbc.username"));
        SOURCE.setPassword(this.config.get("jdbc.password"));
        SOURCE.setMinIdle(5);
    ...
    public List<User> findAll() {
        ...
        try {
            ...
            rs = st.executeQuery("SELECT * FROM store limit 3");
            savepoint = connection.setSavepoint();
            while (rs.next()) {
    ...
    public User findByLogin(String login) {
        ...
        try {
            connection = SOURCE.getConnection();
//            st = connection.createStatement();
            ps = connection.prepareStatement("SELECT * FROM store WHERE login = ?");
            ps.setString(1, login);
            rs = ps.executeQuery();
//            rs = st.executeQuery("SELECT * FROM store WHERE login = " + login + ";");
//            rs = st.executeQuery("SELECT * FROM store WHERE login = " + login + ";");
            savepoint = connection.setSavepoint();
            while (rs.next()) {
                User user = new User(rs.getInt(1), rs.getString(2),
    ...
    public User findByEmail(String email) {
        ...
        try {
            connection = SOURCE.getConnection();
            ps = connection.prepareStatement("SELECT * FROM store WHERE email = ?");
        ...
        try {
            ...
            if (!rs.next()) {
                st.executeUpdate("create table store (id SERIAL primary key, name varchar(20),"
                        + "login varchar(20) UNIQUE, email varchar(20) UNIQUE, password varchar(20), role varchar(20),"
